...
- (void) renderFrameWithState:(const Vuforia::State&) state projectMatrix:(Vuforia::Matrix44F&) projectionMatrix {
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if (trackable.getName()) {
            if (_delegate && [_delegate respondsToSelector:@selector(imageTargetsEAGLView:didScaned:)]) {
                NSString *trackerName = [NSString stringWithCString:trackable.getName() encoding:NSUTF8StringEncoding];
                [_delegate imageTargetsEAGLView:self didScaned:trackerName];
            }
        }
