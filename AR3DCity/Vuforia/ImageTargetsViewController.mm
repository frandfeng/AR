...
//    }];
    ...
    self.backgroundImageView.image = [UIImage imageNamed:@"6"];
    [self.view addSubview:self.backgroundImageView];
    
    self.animateImageView = [[UIImageView alloc] initWithFrame:rect];
    [self.view addSubview:self.animateImageView];
    [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(setNextImage) userInfo:nil repeats:YES];
    ...
    UIButton *flashButton = [[UIButton alloc] initWithFrame:CGRectMake(rect.size.width-60, 10, 50, 50)];
    flashButton.tag = 0;
...
- (void)setNextImage {
    ...
    NSString *imageName = [NSString stringWithFormat:@"00%03d", self.index];
...
//    [self.navigationController popViewControllerAnimated:YES];
    ...
    [[PWUnityMsgManager sharedInstance] sendMsg2UnityOfType:@"OnARScanComplete" andValue:[NSString stringWithFormat:@"{\"params\":{\"state\":\"%@\",\"identifiedName\":\"\"}}", @"1"]];
}
    ...
    if (captureDeviceClass != nil) {
        ...
        if ([device hasTorch]) { // 判断是否有闪光灯
            ...
            if (sender.tag == 0) {
                ...
                [sender setBackgroundImage:[UIImage imageNamed:@"flash_open"] forState:UIControlStateNormal];
                sender.tag = 1;
                [device setTorchMode:AVCaptureTorchModeOn]; // 手电筒开
            }else{
                
                [sender setBackgroundImage:[UIImage imageNamed:@"flash_close"] forState:UIControlStateNormal];
                sender.tag = 0;
...
- (bool) doLoadTrackersData {
    dataSetStonesAndChips = [self loadObjectTrackerDataSet:@"AR_Card.xml"];
    dataSetTarmac = [self loadObjectTrackerDataSet:@"AR_Ticket.xml"];
    dataSetStonesAndChips1 = [self loadObjectTrackerDataSet:@"AR_Sand.xml"];
    if ((dataSetStonesAndChips == NULL) || (dataSetTarmac == NULL) || (dataSetStonesAndChips1 == NULL)) {
        NSLog(@"Failed to load datasets");
        return NO;
    }
    if ((! [self activateDataSet:dataSetStonesAndChips]) || (! [self activateDataSet:dataSetTarmac]) || (! [self activateDataSet:dataSetStonesAndChips1])) {
...
{
//    NSLog(@"onVuforiaUpdate switchToTarmac %d switchToStonesAndChips %d", switchToTarmac, switchToStonesAndChips);
    if (switchToTarmac) {
...
- (void)imageTargetsEAGLView:(ImageTargetsEAGLView *)view didScaned:(NSString *)trackableName {
    NSLog(@"name %@", trackableName);
    if (![self.trackableName isEqualToString:trackableName]) {
        self.trackableName = trackableName;
        [self dismissViewControllerAnimated:NO completion:nil];
        [[PWUnityMsgManager sharedInstance] sendMsg2UnityOfType:@"OnARScanComplete" andValue:[NSString stringWithFormat:@"{\"params\":{\"state\":\"0\",\"identifiedName\":\"%@\"}}", self.trackableName]];
    }
}
